...
any_value((select p.payment_no from payment p where p.id = l.payment_ref_id)) as payment_ref_id
	...
	if (customer_id !== 'all') {
		query = query + ` and l.customer_id = '${customer_id}' `;
	}

	query =
		query +
		`	and ledger_date between '${start_date}' and '${end_date}' 
	 		and l.ledger_detail != 'Invoice Reversal'
	 		group by  l.customer_id, invoice_ref_id, payment_ref_id
	 		order by l.customer_id, ledger_date
  `;
